...
exports.store = async (req, res, next) => {
    ...
    if (req.admin)
        permissionResult = await this.canAdmin(req.admin.roleId, "add category");
    if (!permissionResult) {
...
exports.update = async (req, res, next) => {
    ...
    let permissionResult = false;
    if (req.admin)
        permissionResult = await this.canAdmin(req.admin.roleId, "update category");
    if (!permissionResult) {
        response.setStatus(403).setMessage("fail").setRes("notAllowed");
        return res.status(403).send(response.handler());
    }
...
exports.destroy = async (req, res, next) => {
    ...
    let permissionResult = false;
    if (req.admin)
        permissionResult = await this.canAdmin(req.admin.roleId, "delete category");
    if (!permissionResult) {
        response.setStatus(403).setMessage("fail").setRes("notAllowed");
        return res.status(403).send(response.handler());
    }
